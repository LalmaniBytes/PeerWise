...
import { User as UserIcon, Mail, Key, Edit, Save, X, Trophy as TrophyIcon } from 'lucide-react';
...
function ProfilePage() {
    ...
    const handleUpdateProfile = async () => {
        try {
            ...
            await axios.put(`${API_URL}/profile/update`, updatePayload, { withCredentials: true });
    ...
    const handleUpdateEmail = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`${API_URL}/profile/update-email`, { newEmail: formData.email }, { withCredentials: true });
            toast.success('Email updated successfully! Please verify the new address.');
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            await axios.put(`${API_URL}/profile/update-picture`, formDataPayload, { withCredentials: true });
            ...
            <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
                <Card className="bg-black/50 border-cyan-500/20 backdrop-blur-xl">
                        ...
                        <div className="flex-1 text-center md:text-left space-y-2">
                            {isEditingName ? (
                                ...
                                <>
                                    ...
                                    <CardDescription className="text-gray-400">{userData.realName || 'No real name provided'}</CardDescription>
                    ...
                    <CardContent className="mt-4">
                        <div className="flex items-center space-x-2">
                            <span className="text-sm text-gray-400 font-semibold">Current Rank:</span>
                            <span className="text-lg">{getRankEmoji(userData.claimedRank)} {userData.claimedRank || 'Unranked'}</span>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="bg-black/50 border-cyan-500/20 backdrop-blur-xl">
                    ...
                    <CardContent className="space-y-4">
                        <div className="flex items-center justify-between">
                            ...
                            <Button onClick={() => setIsEmailModalOpen(true)} variant="outline" size="sm" className="border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/10">
                                Update Email
                            </Button>
                        </div>
                        <div className="flex items-center justify-between">
                            ...
                            <Button onClick={() => setIsPasswordModalOpen(true)} variant="outline" size="sm" className="border-cyan-500/30 text-cyan-400 hover:bg-cyan-500/10">
                                Change Password
                            </Button>
                        </div>
                        <div className="flex items-center justify-between">
                            ...
                            <Button onClick={() => navigate('/rewards')} variant="outline" size="sm" className="bg-gradient-to-r from-cyan-500 to-green-500 hover:from-cyan-400 hover:to-green-400 text-black font-semibold">
                                View
                            </Button>
                        </div>
                    </CardContent>
                ...
                <Card className="bg-black/50 border-cyan-500/20 backdrop-blur-xl">
                    ...
                    <CardContent className="grid grid-cols-2 gap-4">
                        <div className="flex flex-col">
                            <span className="text-2xl font-bold text-cyan-400">{userData.questionsAsked || 0}</span>
                            <span className="text-sm text-gray-400">Questions Asked</span>
                        </div>
                        <div className="flex flex-col">
                            <span className="text-2xl font-bold text-cyan-400">{userData.answersGiven || 0}</span>
                            <span className="text-sm text-gray-400">Answers Given</span>
                        </div>
                        <div className="flex flex-col">
                            <span className="text-2xl font-bold text-green-400">{userData.totalCredits || 0}</span>
                            <span className="text-sm text-gray-400">Total Credits</span>
                        </div>
                        <div className="flex flex-col">
                            <span className="text-2xl font-bold text-green-400">{userData.bestAnswerCount || 0}</span>
